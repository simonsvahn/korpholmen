const RELEASE = '2026-08-04-korpholmen-pwa-3';
...
const FALLBACK_SHELL = ['./', './index.html', './styles.css?v=2026-08-04-pwa-1', './app-switcher.css?v=2026-08-04-pwa-1', './manifest.webmanifest', './icons/korpholmen.svg', './icons/korpholmen-180.png', './icons/korpholmen-192.png', './icons/korpholmen-512.png', './src/app.js?v=2026-08-04-pwa-1', './src/config.js', './release-manifest.json'];
...
self.addEventListener('install', event => {
  event.waitUntil(shellRequests().then(requests => caches.open(CACHE).then(cache => cache.addAll(requests))).then(() => self.skipWaiting()));
});
